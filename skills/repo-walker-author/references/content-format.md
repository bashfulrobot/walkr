...
> Bundled copy: this file is a snapshot of `docs/content-format.md` from the
> `repo-walker` repo, shipped inside this skill so it travels with it wherever
> the skill is copied. If you are working inside the `repo-walker` repo itself,
> `docs/content-format.md` is the canonical original — re-sync this copy if
> that file changes. Everything below is otherwise verbatim.
